#!/usr/bin/env bash
#
# Build the dynamic_characterization conda package and upload it to the
# `diepers` channel.
#
# Prerequisites (once):
...
#
# The recipe reads its version from dynamic_characterization/__init__.py, so
# bump that before building a new release.
...
echo "==> Building dynamic_characterization conda package"
