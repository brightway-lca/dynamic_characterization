...
- Runs nbconvert (markdown output) into docs/content/examples/demos/
...
# notebooks/ is flat, so every generated page lands in the same sub-directory
# (and the same section of the Examples nav).
CATEGORY = "demos"
GITHUB_REPO = "https://github.com/brightway-lca/dynamic_characterization"
...
NOTEBOOK_META: dict[str, tuple[str, list[str]]] = {
    "dynamic_characterization_demo.ipynb": (
        "lucide/trending-up",
        ["demo", "dynamic characterization", "radiative forcing", "ipcc"],
    ),
    "prospective_dynamic_lcia_demo.ipynb": (
        "lucide/telescope",
        ["demo", "prospective", "scenario", "gwp"],
    ),
    "fair_dynamic_lcia_demo.ipynb": (
        "lucide/thermometer",
        ["demo", "fair", "scenario"],
    ),
}

# Every notebook here is a demo; saying so again in the page URL adds nothing.
DEMO_SUFFIX = re.compile(r"_demo$")

# Assets that can be copied into the docs and rendered there. Anything else a
# notebook links to (an Excel model, say) is linked on GitHub instead.
...
def tighten_output_blocks(body: str) -> str:
    ...
    Fenced blocks are skipped, so indented content inside them is left alone.
    """
    lines = body.split("\n")
    result: list[str] = []
    in_fence = False
...
def page_path(notebook_rel: str) -> Path:
    ...
    return Path(CATEGORY) / f"{DEMO_SUFFIX.sub('', rel.stem)}.md"
...
def rewrite_target(target: str, notebook_rel: str, copied: set[Path]) -> str | None:
    ...
    - a renderable asset (an image) is copied into docs/content/examples/data/
      and linked there,
    - any other asset becomes a GitHub link.
...
def convert(notebook_rel: str, icon: str, tags: list[str], copied: set[Path]) -> Path:
    ...
    # highlight (see docs/stylesheets/extra.css) - both were tried in bw_timex
    # and both corrupt Zensical's rendering: a wrapping <div markdown> breaks
    ...
    # Zensical's renderer entirely. Scoping is done via JS instead - see
    # docs/javascripts/source-overrides.js.
...
def main() -> None:
    ...
    # Regenerate the category directory from scratch, so a renamed or dropped
    # notebook can't leave a stale page (or its images) behind. The hand-written
    # index.md lives above it and is untouched.
