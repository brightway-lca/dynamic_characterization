...
Two features bring dependencies that aren't installed by default:

| extra | what it adds | install |
|---|---|---|
| `fair` | the [FAIR](fair-path) climate-model path (`fair`, `pooch`) | `uv add "dynamic-characterization[fair]"` |
| `timex` | [`bw_timex`](https://github.com/brightway-lca/bw_timex), for the time-explicit LCA case study in the demo notebooks | `uv add "dynamic-characterization[timex]"` |

  [fair-path]: ../api/fair.md

!!! note

    `bw_timex` requires Python 3.11 or newer, unlike this package. On an older Python the `timex` extra resolves to nothing rather than failing.

With `pip` instead of `uv`, the same extras are `pip install "dynamic-characterization[fair]"`.
